...
| V2 normal-read authority | future control / `walgit-store::v2_control` | one routing-digest-derived strict control GET, then exact rooted catalog versions only when required | PR2 adds a dormant load/Create/exact-CAS adapter with full-key validation, distinct CAS/version binding, exact successor validation, and one-read ambiguity classification. It is not activated by any V1 or runtime path. Only the control CAS can publish semantics; immutable candidates and mutable auxiliary state stay non-authoritative. Capacity, receipts, authorization, fencing, catalogs, and runtime activation remain later slices. | `cargo test -p walgit-store --test v2_control_store`; `cargo test -p walgit-proto --test v2_codec`; future exact-root, authorization, capacity, fencing, cold-ref, stale-host-index, and side-state gate |
