...
| V2 normal-read authority | future control / store | one routing-digest-derived control GET, then exact rooted catalog versions only when required | Not implemented by PR1; only the control CAS publishes semantics, mutable auxiliary state stays non-authoritative, and capability authorization matches the required exact grant issuer/subject pair in current control | Future request-count, exact-version root, exact-grant, cold-ref, stale-host-index, and side-state non-publication gate |
