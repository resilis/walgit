...
| Managed capability auth | config / server | optional `server.auth.managed_tokens` in token/oidc mode | Accept only prefixed, closed v1 EdDSA JWTs from configured public keys for one exact canonical repository and TenantRole, for at most 15 minutes. Preserve Basic and Bearer Git use. Never union ambient grants/operator rights, mint `wgt_`, forward through the principal-only broker, expose siblings in lists, or issue signed bundle/LFS URLs. This is V1 managed authentication and does not activate dormant V2 control. | `cargo test -p walgit-server managed_capability --lib`; exact managed route e2e; `cargo test -p walgit-config managed_token` |
