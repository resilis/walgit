...
| Effective settings secrecy | config / server / CLI | `/{o}/{r}/api[-browser]/settings/{effective,describe,validate}`; effective settings CLI; config dump | Local PR2 blocker closed: settings boundaries use the exact four-section `EffectiveSettingsView`, and config dump uses the separate credential-safe `SafeConfigView`; no boundary serializes runtime `Config`. This is local serialization evidence, not provider or runtime-readiness evidence. | `cargo test -p walgit-config`; `cargo test -p walgit-server --test e2e settings_views_are_exact_and_secret_free_on_both_api_lanes -- --exact`; projection/code review |
| Auth `none` | config / server | `server.auth.mode=none` | Preserve loopback-only validation | `cargo test -p walgit-config` |
| Auth `token` | config / server | bearer/basic static tokens | Local constant-time secret blocker closed: startup resolves once, rejects empty/duplicate resolved values, retains only domain-separated fixed-width digests, and checks every configured digest without match-dependent loop control. Bearer/Basic, OIDC robot precedence, issued tokens, forwarding, and HMAC behavior remain preserved. Repository grants, provider evidence, and runtime readiness remain separate gates. | `cargo test -p walgit-server auth::tests --lib`; `cargo test -p walgit-config`; later repository-grant/provider/runtime gates |
...
| Finite capacity allocation | dormant `walgit-proto`; future global capacity controller / shards | `capacity_control`, one flat tenant page, exactly 256 capacity shards | This slice freezes schema v1, strict codecs/validation, global exact refs, a 4,096-row/512 KiB tenant-page bound, positive checked budgets/slices, current tenant accounts, retained historical reservation proofs, explicit 900-second RESERVED expiry, non-expiring COMMITTING, exact CHARGED/ABORTED proofs, and closed STABLE/DRAINING/APPLYING recovery shapes. APPLYING binds 256 exact drained baselines; final STABLE binds exact successor or restored proofs. Exported cross-object gates separately exact-bind retained epoch-start and mutable current shard bodies, exact-bind loaded page bodies, prove every checked tenant-slice column fits its shard budget and the aggregate fits the global plan, bind every current reservation/account to its page slice, and require STABLE for admission. Composed pre-CAS gates admit legal successors only through the exact STABLE view or terminal-only PREPARING/DRAINING view with caller-supplied time. State-specific CHARGED and conflict gates exact-bind the prior COMMITTING shard and prepared receipt, plus the typed current RepoControl GET and exact receipt catalog; the closed conflict class proves create-key occupancy, same-writer version advance, or a typed-current writer epoch strictly greater than the committing epoch. The future controller must run those gates after exact loads and perform CAS recovery. The multilevel 65,536-row topology, controller, store/provider operations, V1/runtime wiring, migration, and production activation remain deferred behind the greenfield hard cut. | `cargo test -p walgit-proto --test v2_capacity`; `cargo test -p walgit-proto --test v2_descriptor`; future controller/provider fault, cross-shard exhaustion, mixed-epoch, and crash-resume gate |
